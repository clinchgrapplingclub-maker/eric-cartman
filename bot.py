...
async def reopen_ticket_record(channel_id: int):
    await db_execute("""
        UPDATE tickets
        SET status = 'open',
            closed_by = NULL,
...
# =========================================================
# AI ASSISTANT DB HELPERS
# =========================================================
async def get_ai_assistant_config(guild_id: int) -> Optional[dict[str, Any]]:
    cached = ai_assistant_config_cache.get(guild_id)
    if cached is not None:
        return cached
...
async def save_alert_message(guild_id: int, ticket_channel_id: int, alert_message_id: int, alert_channel_id: int):
    await db_execute("""
        INSERT INTO ai_alert_messages (guild_id, ticket_channel_id, alert_message_id, alert_channel_id)
        VALUES ($1, $2, $3, $4)
        ON CONFLICT (ticket_channel_id)
        DO UPDATE SET
            alert_message_id = EXCLUDED.alert_message_id,
            alert_channel_id = EXCLUDED.alert_channel_id
    """, guild_id, ticket_channel_id, alert_message_id, alert_channel_id)


async def get_alert_message(ticket_channel_id: int) -> Optional[dict[str, Any]]:
    try:
        return await db_fetchrow("""
            SELECT guild_id, ticket_channel_id, alert_message_id, alert_channel_id
            FROM ai_alert_messages
...
async def delete_alert_message(ticket_channel_id: int):
    try:
        await db_execute("DELETE FROM ai_alert_messages WHERE ticket_channel_id = $1", ticket_channel_id)
    except Exception:
        pass
...
def ai_is_simple_greeting(text: str) -> bool:
    return normalize_simple_text(text) in {"hi", "hello", "hey", "yo", "sup", "hej"}


def ai_user_wants_close(text: str) -> bool:
    t = normalize_simple_text(text)
    return t in {
        "no", "nope", "nah", "no thanks", "no thank you",
        "nothing else", "all good", "thats all", "that's all",
        "done", "finished", "close", "close it"
    }


def ai_user_says_yes_only(text: str) -> bool:
    return normalize_simple_text(text) in {"yes", "yeah", "yep", "sure", "ok", "okay"}


def clean_prompt_knowledge(prompt_text: str) -> str:
    """
    Prompt channel is knowledge, not ready-made responses.
    Keep useful facts, remove bot workflow/internal instructions.
    """
    if not prompt_text:
        return ""
    ...
    for raw in prompt_text.splitlines():
        ...
        if any(x in low for x in [
            "then you should", "assistant should", "rename ticket", "rename the ticket",
            "call hr", "ping hr", "alert hr", "notify hr",
            "once they", "after they", "check proof", "check so the proof",
            "do these steps", "follow these steps"
        ]):
            continue

        kept.append(line)

    # newest first from fetch; limit to avoid confusing the model
    return "\n".join(kept[:25])


def fallback_helpful_reply(user_text: str, prompt_knowledge: str) -> str:
    t = normalize_simple_text(user_text)

    if ai_is_simple_greeting(user_text):
        return "Hi! How can I help you?"

    if "staff" in t:
        if "staff" in prompt_knowledge.lower() and "closed" in prompt_knowledge.lower():
            return "Unfortunately, staff applications are currently closed. It will be announced when they open again, so stay tuned!"
        return "I can help with staff-related questions. What would you like to know?"

    if "member" in t or "join" in t:
        return "Sure! I can help you with that. Please follow the server’s member instructions, then send proof here when you’re finished."

    if "proof" in t or "done" in t or "finished" in t:
        return "Alright, I’ll get a staff member to double-check it and help finish this."

    return "Tell me what you need help with and I’ll guide you."
...
async def build_reopened_ticket_embed(guild_id: int, reopened_by: discord.Member) -> discord.Embed:
    config = await get_guild_config(guild_id)
    color = hex_to_color(config["color_hex"]) if config else discord.Color.green()

    embed = discord.Embed(
        title="Ticket Reopened",
        description=f"Ticket reopened by {reopened_by.mention}.",
        color=color
    )
    return await apply_footer(embed, guild_id)
...
async def fetch_prompt_from_channel(channel_id: int) -> Optional[str]:
    ...
    try:
        ...
        async for message in channel.history(limit=100):
            ...
            if content:
                # history() default order is newest first, which is what we want
                collected.append(content)

        if not collected:
            return None

        # newest rules first so they override older ones when both match
        return "\n\n".join(collected)
    except Exception:
        return None

async def call_deepseek_api_fast(messages: list[dict], max_tokens: int = 180) -> Optional[str]:
...
async def renameticket(interaction: discord.Interaction, name: str):
    ...
    if not await is_support_or_admin(interaction.user, interaction.guild.id):
        await interaction.response.send_message(
            embed=await base_embed(interaction.guild.id, "Access Denied", "Only the support team or admins can rename tickets.", error=True),
            ephemeral=True
        )
        return
    ...
    if not clean_name:
        await interaction.response.send_message(
            embed=await base_embed(interaction.guild.id, "Error", "Invalid channel name.", error=True),
            ephemeral=True
        )
        return
    ...
    try:
        ...
        await interaction.response.send_message(
            embed=await base_embed(interaction.guild.id, "Ticket Renamed", f"Ticket renamed to `{clean_name}`."),
            ephemeral=True
        )
        await send_log(
            interaction.guild,
            "Ticket Renamed",
            f"Channel: {interaction.channel.mention}\nRenamed to: `{clean_name}`\nBy: {interaction.user.mention}"
        )
    except Exception as e:
        await interaction.response.send_message(
            embed=await base_embed(interaction.guild.id, "Error", f"Failed to rename channel: {e}", error=True),
            ephemeral=True
        )
...
async def enableticketassistant(
    ...
    prompt_embed = discord.Embed(
        ...
        description=(
            ...
            "- newer rules are checked before older rules\n"
...
@app_commands.guild_only()
@app_commands.default_permissions(administrator=True)
async def disableticketassistant(interaction: discord.Interaction):
    if not interaction.guild or not isinstance(interaction.user, discord.Member):
        return
    ...
    await interaction.response.send_message(
        embed=await base_embed(
            ...
            "Ticket assistant has been deactivated."
        ),
        ephemeral=False
    )
...
async def serverstatus(interaction: discord.Interaction):
    ...
    if premium_row:
        ai_status = "Enabled" if (ai_config and ai_config["enabled"]) else "Disabled"
        await interaction.response.send_message(
            embed=await base_embed(
                interaction.guild.id,
                "Server Status",
...
async def premiumservers(interaction: discord.Interaction):
    ...
    if owner_user:
        if not rows:
            ...
            lines = []
            for row in rows:
                ai_config = await get_ai_assistant_config(row["guild_id"])
                ai_status = "✅" if (ai_config and ai_config["enabled"]) else "❌"
                lines.append(
                    f"**{row['guild_name']}**\n"
                    f"ID: `{row['guild_id']}`\n"
...
@renameticket.error
async def renameticket_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    embed = await base_embed(
        interaction.guild.id if interaction.guild else None,
        "Rename Failed",
        f"{error}",
...
@disableticketassistant.error
async def disableticketassistant_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    embed = await base_embed(
        interaction.guild.id if interaction.guild else None,
        "Disable Assistant Failed",
        f"{error}",
        error=True
    )
...
async def get_ai_response_advanced(
    ...
    if ai_config and ai_config.get("prompt_channel_id"):
        fetched_prompt = await fetch_prompt_from_channel(ai_config["prompt_channel_id"])
        if fetched_prompt:
            raw_prompt = normalize_ai_prompt_text(fetched_prompt)
    if not raw_prompt:
...
# =========================================================
# LIVE PROMPT + CLOSE CONFIRMATION OVERRIDES
# =========================================================
def ai_is_negative_reply(text: str) -> bool:
    t = normalize_simple_text(text)
    return t in {
        "no", "nope", "nah", "that's all", "thats all", "nothing else",
...
async def refresh_prompt_cache_for_guild(guild_id: int, prompt_channel_id: int) -> str:
    ...
    normalized = normalize_ai_prompt_text(fetched_prompt or "")
    custom_prompts_cache[guild_id] = normalized
    ai_prompt_runtime_cache[guild_id] = (asyncio.get_running_loop().time(), normalized)
    try:
        await update_custom_prompt(guild_id, normalized)
    except Exception:
        # runtime cache is enough even if DB update fails
        pass
    return normalized
...
async def get_ai_response_advanced(
    ...
    custom_prompt = ""
    if ai_config and ai_config.get("prompt_channel_id"):
        prompt_channel_id = ai_config["prompt_channel_id"]
        cached = ai_prompt_runtime_cache.get(guild_id)
        now = asyncio.get_running_loop().time()
        if cached and now - cached[0] < 1.0:
            custom_prompt = cached[1]
        else:
            custom_prompt = await refresh_prompt_cache_for_guild(guild_id, prompt_channel_id)
    if not custom_prompt:
        custom_prompt = custom_prompts_cache.get(guild_id, "") or ""

    # Prompt rules have highest priority
...
Rules:
1. Respect prompt channel instructions first.
2. Never close a ticket unless the user clearly indicates they are done.
3. Never rename a ticket unless a real staff member needs to step in.
4. If unsure, ask one short clarifying question instead of repeating yourself.
5. Keep replies short, useful, natural, and non-repetitive.
6. Never delete tickets.
...
"""
    ...
    response = await call_deepseek_api_fast(messages, max_tokens=90)
    parsed = extract_json_object(response or "")

    if not parsed:
        return {
            "reply": build_non_repeating_unclear_reply(user_message, conversation[-1] if conversation else None),
            "close_ticket": False,
            "needs_staff": False,
            "staff_summary": "",
            "suggested_category": "",
    ...
    return {
        "reply": reply or build_non_repeating_unclear_reply(user_message, conversation[-1] if conversation else None),
        "close_ticket": close_ticket,
        "needs_staff": needs_staff,
        "staff_summary": staff_summary,
        "suggested_category": suggested_category,
...
async def handle_ai_assistant_message_advanced(message: discord.Message):
    ...
    try:
        ...
        # If we previously asked "Do you need anything else?", wait for the user's actual answer
        if state.get("awaiting_anything_else"):
            state["awaiting_anything_else"] = False
            if ai_is_negative_reply(user_text):
                ...
                cleanup_ticket_channel_lock(channel.id)
                return
            # User asked something new or said yes: continue normally, do not auto-close
        ...
        reply = (ai_data.get("reply") or "").strip()
        close_ticket = bool(ai_data.get("close_ticket", False))
        needs_staff = bool(ai_data.get("needs_staff", False))
        staff_summary = (ai_data.get("staff_summary") or "").strip()
        rename_to = (ai_data.get("rename_to") or "").strip()
        ...
        if needs_staff and not state.get("staff_alerted"):
            opener_member = await try_fetch_member(guild, ticket["opener_id"])
            enriched = ultra_build_staff_summary(user_text, summary)
            if opener_member and ai_config.get("alert_channel_id"):
                await send_staff_alert(
                    guild=guild,
                    ticket_channel=channel,
        ...
        # Ask before closing; do not instantly close after a normal answer
        if not needs_staff and not close_ticket and ai_should_ask_anything_else(reply, needs_staff=needs_staff, close_ticket=close_ticket):
            state["awaiting_anything_else"] = True
            with suppress(Exception):
...
async def on_message(message: discord.Message):
    ...
    if guild:
        ai_config = await get_ai_assistant_config(guild.id)
        if ai_config and ai_config.get("prompt_channel_id") == message.channel.id:
            await refresh_prompt_cache_for_guild(guild.id, message.channel.id)
            return
